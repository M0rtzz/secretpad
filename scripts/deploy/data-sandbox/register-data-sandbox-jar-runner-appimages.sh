#!/bin/bash
#
# Register the Data Sandbox JAR Runner AppImages (data-sandbox-jar-runner + -nonet) into Kuscia.
# Z-05 计算任务运行组件：一次性 Kuscia Job 运行用户 JAR，结果经 scope=Cluster 端口取回。
#
# Requirements:
#   - a running Kuscia master container (all-in-one: root-kuscia-master)
#   - docker available on this host
#   - the jar-runner image built & loaded (data-sandbox-package/docker/data-sandbox-jar-runner/)
#
# Usage:
#   ./register-data-sandbox-jar-runner-appimages.sh [kuscia_master_container]
#
# Overridable environment variables:
#   DATA_SANDBOX_JAR_RUNNER_IMAGE  default: data-sandbox-jar-runner:latest
#
...
DATA_SANDBOX_JAR_RUNNER_IMAGE="${DATA_SANDBOX_JAR_RUNNER_IMAGE:-data-sandbox-jar-runner:latest}"

log() { echo "[data-sandbox-jar-runner-appimages] $*"; }
...
if ! docker ps --format '{{.Names}}' | grep -qx "${KUSCIA_MASTER_CTR}"; then
    log_error() { echo "[data-sandbox-jar-runner-appimages] ERROR: $*" >&2; }
...
apply_appimage "data-sandbox-jar-runner"      "data-sandbox-jar-runner.yaml"      "${DATA_SANDBOX_JAR_RUNNER_IMAGE}"
# 隔离对照变体：无 scope=Cluster 端点，平台无法取回结果（不可达证明）
apply_appimage "data-sandbox-jar-runner-nonet" "data-sandbox-jar-runner-nonet.yaml" "${DATA_SANDBOX_JAR_RUNNER_IMAGE}"

log "data-sandbox-jar-runner AppImages registered (正常 + -nonet 隔离变体); verify with:"
